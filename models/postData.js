...
const getPostById = async id =>
  await db.execute(
    "SELECT post.*,user.fname,user.lname,user.imgurl,DATE_FORMAT(post.date_created,'%b,%d %Y') AS date_formated " +
      "FROM post,user WHERE post_id = ?",
    [id]
  );
...
const getRepliesByPostId = async post_id =>
  await db.execute(
    "SELECT reply.*,user.imgurl FROM reply,user WHERE reply.user_id = user.user_id AND reply.post_id = ? ORDER BY `date_created`",
    [post_id]
  );
